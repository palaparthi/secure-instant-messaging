...
def check_challenge_validity_and_send_response(packet, address):
    ...
    decrypted_text = aes_gcm_decrypt(users_state[username]['key'], encrypted_text, client_iv, client_tag)
    ...
    encrypted_text, iv, encryptor = aes_gcm_encrypt(users_state[username]['key'], str(c2 + 10))
...
def main():
    ...
    while 1:
        ...
        if packet_type == 'SIGN-IN_1':
            ...
            data_to_send = handle_list(data_decode)
            s.sendto(data_to_send.SerializeToString(), (address[0], address[1]))
